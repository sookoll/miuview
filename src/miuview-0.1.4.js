...
(function($) {
    ...
    $.fn.MiuView = function(o){
        ...
        Shadowbox.init({
            ...
            onFinish:function(i){
                if($('.mv-buttons').length==0){
                    ...
                    buttons = '<div id="mv-close" class="mv-buttons"></div><a href="#" id="mv-open-item" class="mv-buttons" target="_blank"></a>';
        ...
        $(this).each(function(){
            ...
            $('#miuview > ul > li a.openItem').live('mouseover', function(){
                $(this).closest('li').find('.caption').show();
            }).live('mouseout', function(){
                $(this).closest('li').find('.caption').hide();
            });
        ...
        function MiuViewG(div){
                ...
                if(hash==''){
                    ...
                    if(count == 0){
                        ...
                        switch(o.request){
                            ...
                            break;
                            case 'getitem':
                                var data = {request:o.request,album:o.album,item:o.item,thsize:o.thsize,size:o.size,key:o.key};
                            break;
                        }
                    ...
                    if(THIS.currentAlbum!=album){
                        ...
                        var req = 'getitem';
                        var data = {request:req,album:album,item:o.item,thsize:o.thsize,size:o.size,key:o.key};
            ...
            this.doGalleryView=function(content,req){
                ...
                if(req=='getitem' && o.request=='getalbum'){
                    ...
                    var back = '<a href="back" class="back" title="Tagasi">< Tagasi</a>';
                }

                $(THIS.div).html('<div id="miuview">'+(back || '')+'<h2>'+title+'</h2><ul>'+content+'</ul><div class="clear"></div></div>');
            }
                    ...
                    case 'getitem':
                        ...
                        for(var i=0;i<data.items_count;i++){
                            ...
                            var link = {
                                ...
                                title : '<b>' + data.items[i].title + '</b> <i>' + data.items[i].description + '</i>',
                ...
                switch(type){
                    case 'getalbum':
                        for(var i in THIS.mvAlbums){
                            ...
                            list+='<div class="caption"><div><b>'+caption+'</b><br>pilte '+THIS.mvAlbums[i].items_count+'</div></div>';
                    ...
                    case 'getitem':
                        for(var i in THIS.mvItems){
                            var title = THIS.mvItems[i].title;
                            var descr = THIS.mvItems[i].description.length > 26?THIS.mvItems[i].description.substring(0,23)+'...':THIS.mvItems[i].description;

                            list+='<li id="'+i+'" class="left"><a href="#'+THIS.mvItems[i].album+'&'+i+'"  class="openItem" title="'+title+'">';
                            if(title.length > 0 || descr.length) {
                                list+='<div class="caption"><div><b>'+title+'</b><br>'+descr+'</div></div>';
                            }
            ...
            this.changeLink = function(id){
                var current = Shadowbox.getCurrent();
                $('#mv-open-item').attr('href', o.url+'/?request=getimage&size=2048&album='+current.gallery+'&item='+current.options.id);
            }
